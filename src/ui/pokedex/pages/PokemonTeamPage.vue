  ...
  >
    <div class="w-full">
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-12 p-10 lg:grid-cols-3">
        <div v-for="pokemon in pokemonSelectedStore" :key="pokemon.id" class="flex justify-center">
          <PokemonCardDetail :key="pokemon.id" :pokemon="pokemon" />
        </div>
      </div>
    </div>
  </section>
...
const { pokemonSelectedStore } = usePokemonSelectedTeam();
...
onMounted(() => {
  ...
  }, 1000); // Gradiente cambia cada segundo
});

onBeforeUnmount(() => {
  clearInterval(gradientInterval); // Limpiar el intervalo al desmontar el componente
});
</script>
